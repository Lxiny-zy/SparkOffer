...
## ⚠️ 实测更新（2026-05-30 baseline 回归）— 方向修正

> 本节是阶段 0「先修尺子」的实测产出，**用数据修正了下方原方案的部分假设**。下方分层方案保留作参考，**优先级以本节为准**。

阶段 0 已在服务器跑出检索 baseline（新建 `backend/eval/rag_recall.py`，user=`0c6f9fc1`，k=5）：

| 路径 | 样本 | HitRate | MRR | Precision | 稳态延迟 | 冷启动 |
|---|---|---|---|---|---|---|
| dense 单 query（`retrieve_topic_context`） | 10 | **1.00** | 0.95 | **0.92** | ~600ms | 12–22s |
| RRF 多路（`retrieve_for_drill`，线上出题主路径） | 7※ | **1.00** | ~0.90 | ~0.60 | **2–3s** | 26–27s |

※ drill 输出 java 三条被终端截断，agent+python 共 7 条，趋势已确定。

**三条结论（数据说话）：**

1. **检索质量已经够用，不是瓶颈。** 两条路径 HitRate 全 1.0、MRR≥0.90。→ 原计划 **P1-E hybrid（#7）/ P1-F rerank（#8）正式搁置**：连现有 RRF 都未超过朴素 dense，更复杂的稀疏融合 / cross-encoder 精排**无收益证据**，且都加延迟。重启条件写进了对应 task（难评测集若暴露"召回到了但排序差"才重启）。

2. **评测集触顶（区分度为零）。** HitRate 全 1.0 = 这把尺子测不出任何短板，也无法验证任何优化。→ **P0-A 扩评测集（#2）升为当务之急**，且要造**更难的区分性 query**：术语歧义 / 近义混淆 / 长尾 / 跨章节组合 / 负样本。原方案"50–80 条"目标不变，但难度要求大幅上调。

3. **真问题是延迟，不是召回。** 冷启动 12–27s（LlamaIndex 索引加载）、RRF 稳态 2–3s（多 query + 语义去重的 embedding 开销）。这才是用户可感知的成本。→ 新增两个务实项：
   - **索引预热 / 冷启动治理**（新 task #12）：lifespan 启动预热共享索引，先用 `rag_recall.py` 的 `ms` 字段定位耗时构成。
   - **审视 RRF 语义去重阈值(0.85) + fallback**（新 task #13）：drill precision 0.60 vs dense 0.92、chunk 数从 5 降到 2–3，疑似去重过激或 fallback 污染。

> 诚实标注：RRF 的低 precision **部分**是"用单 eval query 测它、触发了它的 fallback query"的评测产物，不完全代表真实多-weak_point 出题；但其**延迟代价**与**"质量未超 dense"**是确凿的。task #13 须用难评测集 + 真实多-WP 场景复测再定夺。

**方向从「堆检索优化（hybrid/rerank）」正式转为「造难评测集 + 治延迟 + 审视 RRF 去重」。**
