...
const FEATURES = [
  ...
  },
  {
    ...
    color: "text-tertiary bg-tertiary/12",
    title: "JD 定向备面",
    ...
    <div className="min-h-screen bg-bg flex flex-col">
      <header className="flex items-center justify-between px-6 md:px-10 py-4">
        <div className="flex items-center gap-2.5">
          <CatAvatar size={32} mood="static" />
          <span className="text-lg font-display font-bold text-text">SparkOffer</span>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => setTheme((t: string) => t === "dark" ? "light" : "dark")}>
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10 md:gap-14 relative z-10">
          <div className="flex-1 text-center md:text-left">
            ...
            <h1 className="text-3xl md:text-5xl font-display font-bold leading-tight mb-4 animate-fade-in-up">
              <span className="bg-gradient-to-r from-primary via-[#9B72CF] to-tertiary bg-clip-text text-transparent">
                越练越懂你的
              </span>
              <br />
            ...
            <Button variant="default" size="lg" className="animate-fade-in-up [animation-delay:0.2s]" onClick={() => navigate("/login")}>
              立即开始
              <ArrowRight size={16} />
            </Button>
          </div>
      ...
      <section className="px-6 md:px-10 pb-12 md:pb-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 stagger-children">
            {FEATURES.map((f) => (
              <Card key={f.title} className="hover:shadow-md hover:scale-[1.02] transition-all duration-300 ease-[cubic-bezier(0.2,0,0,1)]">
                <CardContent className="p-6">
                  <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center mb-3", f.color)}>
                    {f.icon}
                  </div>
      ...
      <footer className="mt-auto border-t border-border py-5 text-center text-xs text-dim">
        SparkOffer — AI Mock Interview Coach
      </footer>
    </div>
  );
}
