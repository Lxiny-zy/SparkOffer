...
def update_user_profile(user_id: str, name: str = None, email: str = None) -> dict:
    conn = get_db()
    if name is not None:
        conn.execute("UPDATE users SET name = ? WHERE id = ?", (name.strip()[:50], user_id))
    if email is not None:
        ...
        if is_owner(user_id) and email != settings.default_email.lower().strip():
            raise HTTPException(400, "Owner email is fixed by DEFAULT_EMAIL in .env; change it there")
        if not is_owner(user_id) and email == settings.default_email.lower().strip():
